...
// public key.
func (tx *TX) GenerateKey(key ManagementKey, slot Slot, opts Key) (crypto.PublicKey, error) {
	...
	resp, err := sendTLV(tx.tx, insGenerateAsymmetric, 0, slot.Key,
		tlv.New(0xac,
...
func (tx *TX) PrivateKey(slot Slot, public crypto.PublicKey, auth KeyAuth) (crypto.PrivateKey, error) {
	pp := PINPolicyNever
	...
	case *ecdsa.PublicKey:
		return &ECPPPrivateKey{tx, slot, pub, auth, pp}, nil

	case ed25519.PublicKey:
		return &keyEd25519{tx, slot, pub, auth, pp}, nil

	case *rsa.PublicKey:
		return &keyRSA{tx, slot, pub, auth, pp}, nil
...
func (tx *TX) SetPrivateKeyInsecure(key ManagementKey, slot Slot, private crypto.PrivateKey, policy Key) error {
	...
	if _, err := sendTLV(tx.tx, insImportKey, byte(policy.Algorithm), slot.Key, tvs...); err != nil {
