...
// TX is an exclusive open connection to a TX smart card. While open,
...
func New(tx iso.TX) (pivCard *TX, err error) {
	pivCard = &TX{
		tx:   tx,
		Rand: rand.Reader,
	}
	if err := pivCard.tx.Select(iso.AidPIV); err != nil {
...
func (tx *TX) Version() iso.Version {
	return *tx.version
}

// Serial returns the YubiKey's serial number.
func (tx *TX) Serial() (uint32, error) {
	if tx.version.Major < 5 {
		...
		if err := tx.tx.Select(iso.AidYubicoOTP); err != nil {
			return 0, fmt.Errorf("failed to select YubiKey applet: %w", err)
		}

		defer tx.tx.Select(iso.AidPIV) //nolint:errcheck

		return yubikey.GetSerialNumber(tx.tx)
	}

	resp, err := send(tx.tx, insGetSerial, 0, 0, nil)
	if err != nil {
...
func (tx *TX) Reset() error {
	...
	pinInt, err := rand.Int(tx.Rand, maxPIN)
	if err != nil {
		return fmt.Errorf("failed to generate random PIN: %w", err)
	}

	pukInt, err := rand.Int(tx.Rand, maxPIN)
	if err != nil {
	...
	if _, err = send(tx.tx, insReset, 0, 0, nil); err != nil {
...
func (tx *TX) getVersion() (*iso.Version, error) {
	resp, err := send(tx.tx, insGetVersion, 0, 0, nil)
	if err != nil {
...
func sendTLV(tx iso.TX, ins iso.Instruction, p1, p2 byte, vs ...tlv.TagValue) (tlv.TagValues, error) {
