...
func (tx *TX) AttestationCertificate() (*x509.Certificate, error) {
	return tx.Certificate(SlotAttestation)
}
...
func (tx *TX) Attest(slot Slot) (cert *x509.Certificate, err error) {
	resp, err := send(tx.tx, insAttest, slot.Key, 0, nil)
	if err != nil {
...
func (tx *TX) SupportsAttestation() bool {
	v, _ := iso.ParseVersion("4.3.0")
	return !tx.version.Less(v)
}
